...
import threading
import queue
import subprocess  # BusyBox 명령어 실행용
...
servo = GPIO.PWM(servo_pin, 50)  # 50Hz 서보 PWM
...
def set_servo_angle(angle):
    ...
    print(f"서보 모터 각도 설정: {angle}도 (Duty Cycle: {duty_cycle:.2f}%)")
...
def dl_inference():
    ...
    while True:
        ...
        time.sleep(max(0, fps_delay - (time.time() - start_time)))
