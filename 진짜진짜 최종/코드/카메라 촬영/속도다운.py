...
def set_dc_motor(speed, direction):
    ...
    dc_motor_pwm.ChangeDutyCycle(speed / 2)  # 속도 25%로 조절
    ...
    def run(self):
        while self.running:
            ...
            if keyboard.is_pressed('up'):
                set_dc_motor(25, "forward")
                print("DC motor moving forward...")
                self.camera_handler.should_save_image = True
            elif keyboard.is_pressed('down'):
                set_dc_motor(25, "backward")
                print("DC motor moving backward...")
            else:
                set_dc_motor(0, "forward")

            time.sleep(0.2)  # 반복 속도 조절
